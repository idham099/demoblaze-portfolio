# demoblaze-portfolio

1. Nama Proyek: DemoBlaze Full-Stack Automation Portfolio
   
   "Ini adalah proyek Full-Stack (UI, API, Performance) "
   
3. Tujuan: Membangun Automation Framework E2E yang stabil

   penggunaan CI/CD dan pengujian Non-Fungsional (K6).
   
5. Framework: Cypress (JavaScript/TypeScript), Page Object Model (POM). API: Cypress cy.request() & cy.intercept() / JSONPlaceholder. Performance: K6. CI/CD: GitHub Actions
  
6. Strategi Pengujian: Menggunakan API Mocking/Stubbing untuk menstabilkan test case UI, untuk menunjukkan kemampuan dalam menangani Dependensi dan Arsitektur Testing.
7. Instruksi:
   npm install,
   
   npx cypress open,
   
   perintah k6 run
  
9. Link ke hasil CI/CD GitHub Actions (Status Badge) dan Test Summary Report (Allure/HTML Report)
